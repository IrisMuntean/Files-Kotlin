...
class FileExplorerActivity : AppCompatActivity() {
    ...
    private lateinit var fileAdapter : FilesRecyclerViewAdapter
    ...
    private fun initViewModelObservers() {
        ...
        fileExplorerViewModel.listedFiles.observe(this, Observer {
            fileAdapter.refreshFiles(it)
        })
    }

    private fun initWidgets() {
        //TODO("Initialize the layout manager and adapter of the recycler view widget")
        val action : (FileEntry) -> Unit = {
            fileExplorerViewModel.openFile(it)
        }
        val linearLayoutManager = LinearLayoutManager(this)
        fileAdapter = FilesRecyclerViewAdapter(action)
        rcv_list_dir.apply {
            adapter = fileAdapter
